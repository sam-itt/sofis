...
//    SDL_Rect center_rect = {0,0,0,0};
    ...
    gauge_renderer = renderer_new();
    renderer_add_gauge(gauge_renderer, BASE_GAUGE(hud), 0, 0);
    renderer_add_gauge(gauge_renderer, BASE_GAUGE(panel), 30, 0);
#if USE_SDL_GPU
...
//        base_gauge_render(BASE_GAUGE(txt), elapsed, &(RenderContext){rtarget, &txtrect, NULL});
        ...
        renderer_render(gauge_renderer, elapsed, &(RenderContext){rtarget, &whole, NULL});
#if USE_SDL_GPU
		GPU_Flip(gpu_screen);
#else
