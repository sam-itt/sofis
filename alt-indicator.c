...
static void alt_indicator_render_to(AltIndicator *self, Uint32 dt, SDL_Surface *destination, SDL_Rect *location);
static BaseGaugeOps alt_indicator_ops = {
    .render = (RenderFunc)alt_indicator_render,
    .render_to = (RenderToFunc)alt_indicator_render_to
};
...
static void alt_indicator_draw_qnh_to(AltIndicator *self, SDL_Surface *destination, SDL_Rect *location)
{
    ...
    oloc = (SDL_Rect){
        .x = location->x,
        .y = location->y + BASE_GAUGE(self)->h - 20 -2,
        .h = 21,
        .w = BASE_GAUGE(self)->w
    };
    ...
    view_draw_text(destination, &qloc, number, self->font, &color, &SDL_BLACK);
}
...
//    if(animated_gauge_moving(ANIMATED_GAUGE(self->ladder)) || animated_gauge_moving(ANIMATED_GAUGE(self->odo))){
        ...
        placement[1].x = location->x + (BASE_GAUGE(self->ladder)->w - BASE_GAUGE(self->odo)->w-1) -1; /*The last -1 in there to prevent eating the border*/
